...
async def print_tool():
    tools = await _amap_mcp_tool.get_tools()
    print(f"加载了 {len(tools)} 个工具:")
    for t in tools:
        print(f" - {t.name}: {t.description}")


def get_amap_mcp_tool() -> MultiServerMCPClient:
    """
    获取高德地图MCP工具实例(单例模式)
    ...
    if _amap_mcp_tool is None:
        ...
        "amap-amap-sse": {
            "url": f"https://mcp.amap.com/sse?key={config.AMAP_API_KEY}",
            "transport": "sse"
            }
        })
