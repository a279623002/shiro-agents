...
available_tools = {
    ...
    "get_attraction": attraction.get_attraction,
}
...
try:
    with open('conf.yaml', 'r', encoding='utf-8') as file:
        ...
        MODEL_ID = llmConfig.get('MODEL')
except FileNotFoundError:
    exit("错误:未找到conf.yaml配置文件。")
except yaml.YAMLError as e:
    exit(f"错误:解析conf.yaml文件失败 - {e}")

if API_KEY == "" or BASE_URL == "" or MODEL_ID == "":
    exit("错误:未在conf.yaml中配置LLM.API_KEY、LLM.BASE_URL或LLM.MODEL。")
